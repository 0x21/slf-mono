...
const VerifyPasswordDialog = ({ open, setOpen, onSuccess }: IProps) => {
  const api = useTRPC();
  const queryClient = useQueryClient();
  const { data, isLoading } = useQuery(
    api.authUser.getSudoModeStatus.queryOptions(),
  );

  const mutation = useMutation(api.authUser.verifyPassword.mutationOptions());
